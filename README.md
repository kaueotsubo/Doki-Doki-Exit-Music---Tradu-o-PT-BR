# Doki Doki Exit Music - Tradução PT-BR 🇧🇷

![Status](https://img.shields.io/badge/Status-Completo-green)
![Versão](https://img.shields.io/badge/Versão-1.1-blue)
![Lang](https://img.shields.io/badge/Linguagem-Ren'Py-red)

Tradução completa para Português do Brasil do mod **Doki Doki Exit Music** (Versão Clássica/Original), um dos mods mais famosos e emocionantes para *Doki Doki Literature Club*.

## 📥 Download

[CLIQUE AQUI PARA BAIXAR A TRADUÇÃO COMPLETA (v1.1)](https://github.com/kaueotsubo/Doki-Doki-Exit-Music---Tradu-o-PT-BR/releases/tag/1.0)

> **Aviso:** Este é um mod feito por fãs e não é afiliado ao Team Salvato. Você deve ter completado o jogo original Doki Doki Literature Club antes de jogar este mod. Baixe o jogo original em: [ddlc.moe](http://ddlc.moe).

## 📖 Sobre o Mod

**Exit Music** é focado na personagem Natsuki e oferece uma narrativa profunda, emocional e, muitas vezes, dolorosa. A história diverge do jogo original, explorando temas mais pesados e realistas, acompanhados por uma trilha sonora inspirada na banda Radiohead.

Esta tradução visa trazer essa experiência para os falantes de português, mantendo a fidelidade das emoções e a fluidez dos diálogos.

## ⚠️ Avisos de Conteúdo

Assim como o jogo base, este mod contém temas pesados que podem não ser adequados para todos os públicos, incluindo:
* Depressão e Ansiedade
* Suicídio e Automutilação
* Violência doméstica
* Linguagem imprópria

**Jogue com discrição.**

## 📥 Instalação

Siga os passos abaixo para instalar a tradução:

1. Tenha uma cópia limpa de **Doki Doki Literature Club** instalada no seu computador.
2. Baixe os arquivos deste repositório (clique em `Code` > `Download ZIP`).
3. Vá até a pasta onde o DDLC está instalado.
4. Abra a pasta `game` do DDLC.
5. Copie os arquivos `.rpy` (como `script-exitmusic.rpy`, `poems.rpy`, etc.) deste repositório e cole dentro da pasta `game`, substituindo os arquivos existentes se solicitado.
6. Abra o jogo e divirta-se!

## 🛠️ Detalhes da Tradução

* **História Principal:** ✅ 100% Traduzida
* **Poemas:** ✅ 100% Traduzidos (Incluindo poemas especiais e glitchs)
* **Menus e Interface:** ⚠️ Parcialmente (Menus principais mantidos no original para compatibilidade)
* **Revisão:** Em andamento

## 🤝 Créditos

* **Tradução PT-BR:** Kauê Otsubo / Otsubo
* **Criadores Originais do Mod:** Oliver Norton / Exit Music Team
* **Doki Doki Literature Club:** Dan Salvato & Team Salvato

## 📄 Licença

Este projeto é uma tradução feita de fã para fã, sem fins lucrativos. Todos os direitos dos assets originais pertencem aos seus respectivos criadores.

---
*Gostou da tradução? Deixe uma estrelinha ⭐ neste repositório!*
